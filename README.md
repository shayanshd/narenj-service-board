...
Narenj is a Persian-first, multi-tenant order hand-off for table-service restaurants. A waiter adds an item to a table, the kitchen moves it from new to preparing to ready, and the waiter confirms delivery. The table visit stays open for additional requests until the waiter explicitly ends service and releases it for the next customer.
...
- **Writes:** narrow commands (`add_item`, `transition_item`, `close_order`, `set_availability`) replace whole-order saves. Additions use idempotency keys; updates use versions and return `409` on stale state. Kitchen staff progress preparation; dining-room staff confirm delivery and separately end the table visit. The server refuses closure while any active item remains.
- **AI:** a manager-only endpoint sends bounded branch aggregates—not customer notes or raw order text—to a structured-output model. Product frequency is grouped by menu category, with ties preserved; there is no misleading cross-category “overall bestseller.” Model output is accepted only if it mentions every calculated category leader. The endpoint uses a timeout, no response storage, schema validation, and a deterministic fallback.
